...
const ItemsPriceControler: React.FC<ProductProps> = ({price, id, image, title, quantity}) => { 
  ...
  // const [totalValueProduct, setTotalValueProduct] = React.useState(price)

  const onPut = React.useCallback(async(id: number, body: ProductProps) => {
    try{
  ...
  return(
    <S.ItemsPriceControlerContainer>    
        ...
        <div className="subtotal-area">
          ...
          {quantity && (
          <H3SansPrice>{priceMasked(price * quantity)}</H3SansPrice>
          )}
        </div>
